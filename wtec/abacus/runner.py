"""ABACUS cluster pipeline: SCF -> NSCF -> Wannier90."""
...
class AbacusPipeline:
    """Orchestrate ABACUS + Wannier90 stages via qsub/mpirun."""
    ...
    def __init__(
        ...
        *,
        ...
        self.executable = str(executable or "abacus").strip() or "abacus"
    ...
    def run_scf(self) -> dict[str, Any]:
        ...
        cmds = [
            ...
            f"if [ -f OUT.{self.material}/running_scf.log ]; then cp OUT.{self.material}/running_scf.log running_scf.log; fi",
        ]
        ...
        return self.jm.submit_and_wait(
            ...
            retrieve_patterns=[scf_out.name, "running_scf.log", f"scf_{self.material}.log", "OUT.*/*.log"],
    ...
    def run_nscf(self, _fermi_ev: float | None = None) -> dict[str, Any]:
        ...
        cmds = [
            ...
            f"if [ -f OUT.{self.material}/running_nscf.log ]; then cp OUT.{self.material}/running_nscf.log running_nscf.log; fi",
        ]
        script = generate_script(
            PBSJobConfig(
        ...
        return self.jm.submit_and_wait(
            ...
            remote_dir=remote_dir,
            local_dir=self.run_dir,
            retrieve_patterns=[nscf_out.name, "running_nscf.log", f"nscf_{self.material}.log", "OUT.*/*.log"],
    ...
    def run_wannier(
        ...
        *,
        ...
        remote_dir = self._remote_dir()
        self._preflight(["qsub", "qstat", "mpirun", self.executable, "wannier90.x"], [*list(files.values()), win])
        queue = self._resolved_queue_name()
        cores_per_node = self._resolved_cores_per_node()
        mpi = MPIConfig(n_cores=self.n_nodes * cores_per_node)

        cmds = [
            "cp INPUT.wannier INPUT",
            "cp KPT.nscf KPT",
            build_command(self.executable, output_file=f"{self.material}.wannier.abacus.out", mpi=mpi),
        ...
        meta = self.jm.submit_and_wait(
            ...
            local_dir=self.run_dir,
            retrieve_patterns=[
                f"{self.material}.wannier.abacus.out",
    ...
    def run_full(self) -> Path:
        ...
        if not hr.exists():
            raise FileNotFoundError(f"_hr.dat not found after ABACUS/Wannier stage: {hr}")
        return hr

    def _input_generator(self) -> AbacusInputGenerator:
        return AbacusInputGenerator(
            atoms=self.atoms,
            material_name=self.material,
            pseudo_dir=self.pseudo_dir,
            orbital_dir=self.orbital_dir,
            pseudopots=dict(getattr(self.preset, "abacus_pseudopots", {}) or {}),
            orbitals=dict(getattr(self.preset, "abacus_orbitals", {}) or {}),
            kpoints_scf=self.kpoints_scf,
            kpoints_nscf=self.kpoints_nscf,
            basis_type=self.basis_type,
            ks_solver=self.ks_solver,
            spin_orbit=True,
            num_bands=max(int(self.preset.num_bands), 2 * int(self.preset.num_wann)),
        )
    ...
    def _preflight(self, required_commands: list[str], local_inputs: list[Path]) -> None:
        ...
        pp = sorted(set((getattr(self.preset, "abacus_pseudopots", {}) or {}).values()))
        orb = sorted(set((getattr(self.preset, "abacus_orbitals", {}) or {}).values()))
        if pp:
            self.jm.ensure_remote_files(self.pseudo_dir, pp)
        if orb:
            self.jm.ensure_remote_files(self.orbital_dir, orb)
        for p in local_inputs:
            pp_local = Path(p)
            if not pp_local.exists() or pp_local.stat().st_size == 0:
                raise FileNotFoundError(f"Missing required local ABACUS input file: {pp_local}")
