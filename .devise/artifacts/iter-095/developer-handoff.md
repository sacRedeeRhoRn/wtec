# Iteration 95 Developer Handoff

## What Changed

- Product commit: `68b2878` (`Default kwant benchmark to grouped thicknesses`)
- Current handoff commit target before this file is committed: `68b2878`
...
- reverted the default `world_size >= len(grouped_tasks)` Kwant benchmark scheduler back to one rank per thickness group
- kept the thin-group fanout logic behind an explicit env override:
  - `TOPOSLAB_KWANT_BENCH_SPLIT_THICKNESS_GROUPS=1`
- rationale: fresh remote evidence from the thin-group fanout run showed no first completed checkpoint row and severe real node memory pressure, so the safer default is to preserve thickness-local reuse by default and require explicit opt-in for the more aggressive split mode
...
I retired the thin-group fanout run and relaunched the continuity root on product commit `68b2878`.
...
Compared to the thin-group fanout run, the grouped-thickness relaunch reduced observed node memory pressure from about `270 GiB` used to about `20 GiB` used on `n019`.

Interpretation:

- the grouped-thickness default is live on the real cluster path
- the relaunch preserves the real runtime contract while materially reducing duplicated-memory pressure
- however the local authority state is still flat at handoff time: no completion beyond `5/35`, no new shard files, and no fresh local `done` line yet
